...
class CalendarioTiendaScreen extends StatefulWidget {
  ...
  const CalendarioTiendaScreen({super.key, required this.id_tienda});

  @override
  _CalendarioTiendaScreenState createState() => _CalendarioTiendaScreenState();
}

class _CalendarioTiendaScreenState extends State<CalendarioTiendaScreen> {
  DateTime _mesActual = DateTime(DateTime.now().year, DateTime.now().month);

  List<Map<String, dynamic>> _todosLosPedidos = [];
  ...
  Future<void> _obtenerpedidos() async {
    ...
    if (response.statusCode == 200) {
      ...
      // 🔹 2. CREAR LISTA FINAL SUMANDO LA LISTA "productos" A CADA PEDIDO
      List<Map<String, dynamic>> pedidosFinales =
          pedido.map<Map<String, dynamic>>((p) {
        final id = p["idpedido"].toString();
        return {
          ...p,
          "productos": agrupado[id], // 👈 LISTA REAL AGRUPADA
        };
      }).toList();

      // 🔹 3. GUARDAR EN LA VARIABLE DE ESTADO
      setState(() {
        _todosLosPedidos = pedidosFinales;
      });
    }
  }
  ...
  Future<void> enviado(id, List productos) async {
    ...
    try {
      ...
      mostrarMensajeFlotante(
        context,
        "❌ Error: No se pudo marcar el pedido como enviado: $e",
      );
    }
  }
  ...
  Future<void> recibido(id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        mostrarMensajeFlotante(
          context,
          "❌ Error: No se pudo marcar el pedido como recibido.",
          colorFondo: Colors.white,
          colorTexto: Colors.redAccent,
      ...
      mostrarMensajeFlotante(
        context,
        "❌ Error: No se pudo marcar el pedido como recibido: $e",
      );
    }
  }
  ...
  Future<void> Norecibido(id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        mostrarMensajeFlotante(
          context,
          "✅ Pedido marcado como no recibido",
          colorFondo: const Color.fromARGB(255, 243, 243, 243),
          colorTexto: const Color.fromARGB(255, 0, 0, 0),
        );
        ...
        mostrarMensajeFlotante(
          context,
          "❌ Error: No se pudo marcar el pedido como no recibido.",
          colorFondo: Colors.white,
          colorTexto: Colors.redAccent,
      ...
      mostrarMensajeFlotante(
        context,
        "❌ Error: No se pudo marcar el pedido como no recibido: $e",
      );
    }
  }
  ...
  Widget _gridDias() {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final citasDelDia = _todosLosPedidos.where((cita) {
  ...
  for (var cita in citas) {
    String idPedido = cita["idpedido"].toString();
          ...
          children: [
            ...
            if (citas.isEmpty)
              const Padding(
                ...
                child: Text("No hay pedidos para este día"),
              )
            else 
              Flexible(
                      ...
                      return Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // 🔵 TÍTULO DEL PEDIDO
                            Text(
                              "Pedido realizado por $nombreCliente",
                              style: const TextStyle(
                                fontSize: 20,
                                fontWeight: FontWeight.bold,
                              ),
                            ...
                            // 🔵 TARJETAS DE LOS PRODUCTOS DEL MISMO PEDIDO
                            Column(
                              children: productos.map<Widget>((cita) {
                                return _tarjetaProducto(cita);   // tu tarjeta actual
                              }).toList(),
                            ),
                                ...
                                RichText(
                                  text: TextSpan(
                                    children: [
                                      ...
                                      TextSpan(
                                        text: primerItem["direccion_envio"] ?? "N/A",
                                ...
                                RichText(
                                  text: TextSpan(
                                    children: [
                                      ...
                                      TextSpan(
                                        text: primerItem["metodo_pago"] ?? "N/A",
                                        style: const TextStyle(color: Colors.black),
                                      ),
                                    ],
                                  ),
                            ...
                            Row(
                              ...
                              children: [
                                Text(
                                  "Total Pedido:  ${NumberFormat('#,###', 'es_CO').format(totalPedido)}",
                                  style: const TextStyle(
                                    fontSize: 18,
                                    fontWeight: FontWeight.bold,
                                    color: Colors.black87,
                            ...
                            if (estado == "pendiente")
                              Row(
                                ...
                                children: [
                                  ...
                                  ElevatedButton.icon(
                                    onPressed: () {
                                      mostrarConfirmacion(
                                        ...
                                        onConfirmar: () => cancelarpedido(primerItem["idpedido"]),
                                      );
                                    },
                                  ...
                                  ElevatedButton.icon(
                                    onPressed: () {
                                      final productos = primerItem["productos"]; 
                                      mostrarConfirmacion(
                                        context,
                                        mensaje: "¿Confirmas que enviaste este producto?",
                                        onConfirmar: () => enviado(
                                          primerItem["idpedido"],
                                          productos, // 🔹 Enviamos lista completa
                                        ),
                                      );
                                    },
                                    icon: Image.asset("assets/enviar.png", width: 20, height: 20),
                                    label: const Text("Enviado", style: TextStyle(color: Colors.white)),
                                    style: ElevatedButton.styleFrom(
                                ...
                                children: [
                                  ...
                                  ElevatedButton.icon(
                                    onPressed: () {
                                      mostrarConfirmacion(
                                      context,
                                      mensaje: "¿Estás seguro de que el cliente no recibio este pedido?",
                                      onConfirmar: () => Norecibido(primerItem["idpedido"]),
                                      );
                                    },
                                    icon: Image.asset("assets/Cancelar.png", width: 20, height: 20),
                                    label: const Text("No recibido", style: TextStyle(color: Colors.white)),
                                    style: ElevatedButton.styleFrom(
                                  ...
                                  ElevatedButton.icon(
                                    onPressed: () {
                                      mostrarConfirmacion(
                                      context,
                                      mensaje: "¿Confirmas que ya el cliente recibio su pedido?",
                                      onConfirmar: () => recibido(primerItem["idpedido"]),
                                      );
                                    },
                                    icon: Image.asset("assets/Correcto.png", width: 20, height: 20),
                                    label: const Text("Recibido", style: TextStyle(color: Colors.white)),
                                    style: ElevatedButton.styleFrom(
